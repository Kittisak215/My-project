...
const RoleBadge = ({ role }) => {
    ...
    const r = roles[role] || { text: role, colors: 'bg-slate-100 text-slate-700' };
    return <span className={clsx('px-2.5 py-1 rounded-full text-xs font-medium', r.colors)}>{r.text}</span>;
}
...
export default function UsersPage() {
    ...
    const fetchUsers = async () => {
        ...
        } catch { toast.error('โหลดข้อมูลผู้ใช้งานไม่สำเร็จ'); }
        setLoading(false);
    };
        ...
        <div>
            <div className="flex flex-col md:flex-row justify-between items-stretch md:items-center bg-white p-4 rounded-xl shadow-sm mb-4 md:mb-6 gap-3 border border-slate-100">
                <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                    ...
                    <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)} className="border border-slate-300 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-blue-500">
            ...
            <div className="md:hidden space-y-3 mb-4">
                ...
                {!loading && filteredUsers.length === 0 && <p className="text-center py-8 text-slate-400">ไม่พบข้อมูล</p>}
                {filteredUsers.map(u => (
                    <div key={u.user_id} className={clsx('bg-white rounded-xl border border-slate-200 p-4 shadow-sm', !u.is_active && 'opacity-70')}>
                        <div className="flex items-start justify-between">
                            <div className="flex-1 min-w-0">
                                ...
                                {u.driver && <p className="text-xs text-blue-600 mt-1">พขร. ID: #{u.driver.driver_id}</p>}
                            </div>
                            <button
                                onClick={() => openEdit(u)}
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left whitespace-nowrap">
                        <thead className="bg-slate-50 text-slate-600 border-b border-slate-200"><tr>
                            ...
                            <th className="px-6 py-4 font-semibold text-center">ระดับผู้ใช้งาน</th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            ...
                            {!loading && filteredUsers.length === 0 && <tr><td colSpan={6} className="text-center py-8 text-slate-400">ไม่พบข้อมูล</td></tr>}
                            {filteredUsers.map(u => (
                                <tr key={u.user_id} className={clsx('hover:bg-slate-50 transition-colors', !u.is_active && 'opacity-70')}>
                                    <td className="px-6 py-4 text-center text-slate-500">#{u.user_id}</td>
                                    <td className="px-6 py-4 font-medium text-slate-800">{u.username}</td>
                                    <td className="px-6 py-4">
                                        <div className="text-slate-800">{u.full_name}</div>
                                        {u.driver && <div className="text-xs text-blue-600">พขร. ID: #{u.driver.driver_id}</div>}
                                    </td>
                                    <td className="px-6 py-4 text-center"><RoleBadge role={u.role} /></td>
                                    <td className="px-6 py-4 text-center"><StatusBadge isActive={u.is_active} /></td>
                ...
                <Modal title="แก้ไขระดับผู้ใช้งาน" onClose={() => setShowModal(false)}>
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                        <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 mb-4">
                            <p className="text-sm font-medium text-slate-800">{editing.full_name}</p>
                            <p className="text-xs text-slate-500">@{editing.username}</p>
                        </div>
                        <div><label className="text-sm font-medium text-slate-700">ระดับผู้ใช้งาน</label>
                            <select {...register('role')} className="mt-1 block w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500">
                                <option value="DRIVER">ผู้ขับรถ (Driver)</option>
                                <option value="EXECUTIVE">ผู้บริหาร (Executive)</option>
                                <option value="ADMIN">ผู้ดูแลระบบ (Admin)</option>
                            </select>
                        </div>
                        <div><label className="text-sm font-medium text-slate-700">สถานะระบบ</label>
