...
export default function GaragesPage() {
  ...
  const openCreate = () => {
    setEditing(null);
    reset({ specialization: ["GENERAL"], is_active: "true" });
    setShowModal(true);
  };
  const openEdit = (g) => {
    setEditing(g);
  ...
  return (
    <div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-stretch md:items-center bg-white p-4 rounded-xl shadow-sm mb-4 md:mb-6 gap-3 border border-slate-100">
        ...
        <button
          onClick={openCreate}
          className="bg-[#8A1ABA] hover:bg-[#72159c] text-white px-5 py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-2 shrink-0"
        >
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex-1 min-w-0">
                ...
                {[g.address, g.district, g.province].filter(Boolean).length >
                  0 && (
                  <p className="text-xs text-slate-400 mt-1 truncate">
                    {[g.address, g.district, g.province]
                      .filter(Boolean)
                      .join(" ")}
                  </p>
                )}
      ...
      <div className="hidden md:block bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 text-sm text-slate-500">
          แสดง {garages.length} จากทั้งหมด {total} รายการ
        </div>
      </div>
      ...
      {showModal && (
        <Modal
          title={editing ? "แก้ไขอู่/ศูนย์บริการ" : "เพิ่มอู่/ศูนย์บริการ"}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              <label className="text-sm font-medium text-slate-700 mb-2 block">
                ความเชี่ยวชาญ (เลือกได้หลายอย่าง)
              </label>
