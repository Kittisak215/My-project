...
export default function VehiclesPage() {
  ...
  const openCreate = () => {
    setEditing(null);
    reset({ status: "READY", is_active: "true" });
    setShowModal(true);
  };
  const openEdit = (v) => {
    setEditing(v);
  ...
  const onSubmit = async (data) => {
    try {
      const payload = {
        ...
        oil_change_interval_km: data.oil_change_interval_km
          ? parseInt(data.oil_change_interval_km)
          : null,
        tire_change_interval_km: data.tire_change_interval_km
          ? parseInt(data.tire_change_interval_km)
          : null,
  ...
  return (
    <div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-stretch md:items-center bg-white p-4 rounded-xl shadow-sm mb-4 md:mb-6 gap-3 border border-slate-100">
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          <div className="relative flex-1 sm:flex-none">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-[#8A1ABA] w-full sm:w-56"
            />
          </div>
          <select
            value={status}
        ...
        <button
          onClick={openCreate}
          className="bg-[#8A1ABA] hover:bg-[#72159c] text-white px-5 py-2 rounded-lg text-sm font-medium shadow-sm transition-colors flex items-center gap-2 justify-center"
        >
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-slate-600">
                  {v.vehicleType?.type_name || "ไม่ระบุ"} • {v.brand} {v.model}
                </p>
                ...
                <div className="flex flex-wrap gap-x-3 gap-y-1 mt-2 text-xs text-slate-500">
                  ...
                  <span>
                    น้ำมัน: {v.oil_change_interval_km?.toLocaleString() || "-"}{" "}
                    กม.
                  </span>
                  <span>
                    ยาง: {v.tire_change_interval_km?.toLocaleString() || "-"}{" "}
                    กม.
                  </span>
              ...
              {vehicles.map((v) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-slate-600">
                    {v.vehicleType?.type_name || "ไม่ระบุ"}
                  </td>
                  ...
                  <td className="px-4 py-3 text-right text-xs">
                    <div className="text-slate-600">
                      ...
                      {v.oil_change_interval_km?.toLocaleString() ||
                        v.vehicleType?.oil_change_interval_km?.toLocaleString() ||
                        "-"}{" "}
                      กม.
                    </div>
                    <div className="text-slate-600">
                      <span className="text-slate-400 font-medium">ยาง:</span>{" "}
                      {v.tire_change_interval_km?.toLocaleString() ||
        ...
        <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex items-center justify-between text-sm">
          ...
          <div className="flex space-x-1">
            <button
              ...
              className="px-3 py-1 border rounded bg-white text-slate-700 hover:bg-slate-100 disabled:opacity-50"
            >
              ก่อนหน้า
            </button>
      ...
      {showModal && (
        <Modal
          title={editing ? "แก้ไขยานพาหนะ" : "เพิ่มยานพาหนะ"}
        ...
        >
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  {...register("license_plate", {
                    ...
                    pattern: {
                      value: /.+-.+/,
                      message: "ต้องมีเครื่องหมาย - คั่น",
                    },
                  })}
              ...
              <div>
                ...
                <input
                  ...
                  className="mt-1 block w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500 placeholder-slate-400"
                />
              </div>
              <div>
                <label className="text-sm font-medium text-slate-700">
                  ระยะถ่ายน้ำมันเครื่อง (กม.)
                </label>
                <input
                  {...register("oil_change_interval_km")}
                  type="number"
                  placeholder="ค่าเริ่มต้นตามประเภทรถ"
                  className="mt-1 block w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500 placeholder-slate-400"
                />
              </div>
              <div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t">
              ...
              >
                {editing ? "บันทึกการแก้ไข" : "เพิ่มยานพาหนะ"}
              </button>
            </div>
          </form>
        </Modal>
