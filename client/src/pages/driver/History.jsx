...
import { History, Wrench, Calendar, ChevronLeft, ChevronRight, MapPin, DollarSign, Clock } from 'lucide-react';
...
export default function DriverHistoryPage() {
    ...
    useEffect(() => {
        ...
        api.get('/repairs', { params: { page: 1, limit: 10 } })
            ...
            .catch(() => { if (isMounted) toast.error('โหลดข้อมูลไม่สำเร็จ'); })
    ...
    const getStatusInfo = (status) => {
        const map = {
            ...
            AWAITING_APPROVAL: { label: '⏳ รออนุมัติงบ', cls: 'bg-purple-50 text-purple-700 border-purple-200/80' },
    ...
    const isOverdueEst = (estimatedEndDate, status) => {
        if (!estimatedEndDate || status === 'COMPLETED' || status === 'REJECTED') return false;
        ...
        <div className="max-w-4xl mx-auto space-y-6 pb-12">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs">
                <div className="flex items-center gap-3.5">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-slate-800">ประวัติการซ่อมบำรุง</h1>
                                        ...
                                        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                                            ...
                                            {r.garage && (
                                                <div className="flex items-start gap-2 text-xs text-slate-600 bg-slate-50 px-3 py-2.5 rounded-xl border border-slate-100">
                                                    <MapPin size={13} className="text-[#8A1ABA] mt-0.5 shrink-0" />
                                                    <div>
                                                        <p className="font-semibold text-slate-700">{r.garage.garage_name}</p>
                                                        {r.garage.phone && <p className="text-slate-400">{r.garage.phone}</p>}
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            {(r.repair_start_date || r.estimated_end_date) && (
                                                <div className="flex items-start gap-2 text-xs text-slate-600 bg-slate-50 px-3 py-2.5 rounded-xl border border-slate-100">
                                                    ...
                                                    <div>
                                                        {r.repair_start_date && (
                                                            <p>เข้าซ่อม: <span className="font-semibold">{formatDate(r.repair_start_date)}</span></p>
                                                        )}
                                                        {r.estimated_end_date && (
                                                            <p className={overdue ? 'text-red-600 font-bold' : ''}>
                                                                {r.status === 'COMPLETED' ? 'เสร็จเมื่อ:' : 'คาดว่าจะเสร็จ:'}
                                                                {' '}<span className="font-semibold">{formatDate(r.estimated_end_date)}</span>
                                                                {overdue && ' ⚠️ เลยกำหนด'}
                                                            </p>
                                                        )}
                                                        {r.repair_end_date && r.status === 'COMPLETED' && (
                                                            <p>เสร็จจริง: <span className="font-semibold text-emerald-700">{formatDate(r.repair_end_date)}</span></p>
                                                        )}
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            {r.mileage_at_repair ? (
                                                <div className="flex items-center gap-2 text-xs text-slate-600 bg-slate-50 px-3 py-2.5 rounded-xl border border-slate-100">
                                                    ...
                                                    <span>เลขไมล์ขณะซ่อม: <span className="font-semibold">{r.mileage_at_repair.toLocaleString()} กม.</span></span>
                                                </div>
                                            ) : null}
                                        </div>
                                        ...
                                        {r.estimated_cost && (
                                            <div className="bg-amber-50 border border-amber-200/60 rounded-xl px-4 py-2.5 mb-2">
                                                ...
                                                <p className="font-bold text-amber-800 text-base mt-0.5">
                                                    ฿{parseFloat(r.estimated_cost).toLocaleString('th-TH', { minimumFractionDigits: 2 })}
                                                </p>
                                            </div>
                                        )}
                                        {r.total_cost && (
                                            <div className="bg-slate-50 border border-slate-200/60 rounded-xl px-4 py-2.5">
                                                ...
                                                <p className="font-extrabold text-slate-800 text-lg mt-0.5">
                                                    ฿{parseFloat(r.total_cost).toLocaleString('th-TH', { minimumFractionDigits: 2 })}
                                                </p>
                                            </div>
                                        )}
                                    </div>
