...
import { Car, Gauge, Wrench, History, LogOut, Menu, Bell, X, CheckCircle, AlertTriangle, Info, XCircle, ChevronRight } from 'lucide-react';
    ...
    const fetchNotifications = useCallback(async () => {
        ...
        try {
            ...
            if (!lastRead) {
                ...
                count = data.filter(n => new Date(n.created_at).getTime() > lastReadTime).length;
            }
        ...
        import('socket.io-client').then(({ io }) => {
            ...
            socket.on('new_notification', () => {
                ...
                import('react-toastify').then(({ toast }) => {
                    toast.info('มีการแจ้งเตือนใหม่', { position: "bottom-right", autoClose: 3000 });
                });
            });
    ...
    return (
        <div className="relative" ref={panelRef}>
            ...
            <button
                ...
                title="การแจ้งเตือน"
            >
                <Bell size={20} className={clsx('transition-colors', unreadCount > 0 ? 'text-[#8A1ABA]' : 'text-slate-400')} />
                {unreadCount > 0 && (
                ...
                <div className="absolute right-0 top-full mt-2 w-80 sm:w-96 bg-white rounded-2xl shadow-xl border border-slate-200 z-50 overflow-hidden">
                    <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-slate-50">
                        <h3 className="font-bold text-slate-800 text-sm flex items-center gap-2">
                            ...
                            การแจ้งเตือน
                            {unreadCount > 0 && (
                                <span className="text-[10px] font-bold bg-red-100 text-red-600 px-1.5 py-0.5 rounded-full">{unreadCount} ใหม่</span>
                            )}
                        </h3>
                    ...
                    <div className="max-h-[420px] overflow-y-auto divide-y divide-slate-100">
                        {loading && notifications.length === 0 && (
                            <div className="py-8 text-center text-slate-400 text-sm">กำลังโหลด...</div>
                        )}
                        {!loading && notifications.length === 0 && (
                        ...
                        {notifications.map(n => (
                            <div key={n.id} className={clsx('p-3.5 flex items-start gap-3 hover:bg-slate-50 transition-colors', bgMap[n.type] || 'bg-white')}>
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    {n.category === 'REPAIR_STATUS' && n.estimated_end_date && (
                                        <p className="text-[11px] text-slate-400 mt-0.5">
                                            คาดเสร็จ: {new Date(n.estimated_end_date).toLocaleDateString('th-TH')}
                                        </p>
                                    )}
                                    {n.category === 'MILEAGE_REMINDER' && (
                                        <a href="/driver/mileage" onClick={() => setOpen(false)} className="inline-flex items-center gap-1 mt-1 text-[11px] font-semibold text-[#8A1ABA] hover:underline">
...
export default function DriverLayout() {
    ...
    const displayName = user?.full_name || user?.driver?.full_name || user?.username || 'Driver';
                    ...
                    <div className="flex items-center gap-3 ml-auto">
                        ...
                        <div className="flex items-center gap-3 pl-3.5 pr-2 py-1.5 rounded-2xl bg-slate-50 border border-slate-200/80 shadow-2xs">
                            <div className="text-right hidden sm:block">
                                ...
                                <span className="inline-flex items-center text-[10px] font-medium text-teal-700 bg-teal-100/80 px-2 py-0.5 rounded-md mt-0.5">
                                    พนักงานขับรถ (Driver)
                                </span>
                            </div>
                            <div className="w-9 h-9 rounded-xl bg-linear-to-tr from-teal-600 to-emerald-500 text-white flex items-center justify-center font-bold text-sm shadow-xs ring-2 ring-teal-100">
                                {initial}
                            </div>
                        </div>
                    </div>
